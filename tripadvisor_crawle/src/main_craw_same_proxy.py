"""
TripAdvisor Reviews Scraper — Crawlee Edition (Shared Browser/Context)
══════════════════════════════════════════════════════════════════════════════

Architecture: ONE shared Camoufox browser + ONE shared context for ALL places.
Proxy and browser are kept the same across places; only rotated when blocked.

  What Crawlee provides:
    • Request queue (place URLs)
    • Concurrency control (max_concurrency=1, sequential)
    • Automatic URL-level retries (max_request_retries)

  What we manage manually:
    • Browser lifecycle  — single AsyncNewBrowser + context via _setup_browser()
    • Proxy session      — one session_id for all places; new session_id on rotation
    • Page lifecycle     — new page per place from shared context; closed after each
    • Captcha detection  — 12-second polling window in scrape_place()
    • GraphQL fetches    — parallel asyncio.gather per round
    • Review parsing & dataset push batching

Proxy rotation logic:
    • On CaptchaBlockedError with retries remaining → _setup_browser(rotation+1)
      closes old browser/context, opens fresh Camoufox with a new proxy session.
    • On final retry exhausted → log error, re-raise (Crawlee marks request failed).
    • Between places with no block → same browser + context + proxy session reused.
"""
...
async def main() -> None:
    async with Actor:
        ...
        # ── Proxy ─────────────────────────────────────────────────────────────
        # We resolve the proxy URL ourselves (one shared session for all places).
        # Proxy only changes on CaptchaBlockedError via _setup_browser(rotation+1).
        ...
        # max_request_retries = N means 1 initial attempt + N retries = N+1 total.
        # Without residential proxy datacenter IPs are always blocked — no point retrying.
        ...
        seq_counter = [0]            # stable sequential numbering independent of retries
        ...
        # ── Shared browser/context state ──────────────────────────────────────
        # One Camoufox browser + one context reused for ALL places.
        # _setup_browser(rotation) closes the old browser/context and opens a fresh
        # one with a new proxy session (session_id = "run_s{rotation+1}").
        _bs: dict = {}

        async def _setup_browser(rotation: int) -> None:
            if _bs.get("context"):
                try:
                    await _bs["context"].close()
                except Exception:
                    pass
            if _bs.get("browser"):
                try:
                    await _bs["browser"].close()
                except Exception:
                    pass
            ...
            headless = _os.environ.get("APIFY_IS_AT_HOME") == "1"
            proxy_str = ", ".join(proxy_groups) if proxy_groups else "NONE"
            Actor.log.info(
                f"  Browser: Camoufox (Firefox) | {vp['width']}×{vp['height']} | "
                f"proxy={proxy_str} | session={session_id}"
            )
            ...
            ctx_kwargs: dict = {"viewport": vp, "locale": "en-US"}
            if proxy_setting:
        ...
        # ── Crawlee BasicCrawler — request queue + retries only ───────────────
        # Browser lifecycle is managed manually above; Crawlee handles request
        # scheduling and retry counting only.
        crawler = BasicCrawler(
            # Crawlee retries the request when scrape_place() raises CaptchaBlockedError.
            max_request_retries=max_retries,
            # Process one place at a time (sequential; one shared context cannot parallelise).
        ...
        async def handle_place(context: BasicCrawlingContext) -> None:
            ...
            # Stable sequential number for this URL (retries reuse the same number).
            ...
            Actor.log.info(
                f"  proxy={proxy_groups_str} | session={_bs.get('session_id', 'run_s1')}"
                f"{attempt_str}"
            )
            ...
            if attempt > 1:
                ...
                await Actor.set_status_message(
                    f"Place {current_seq}/{len(place_urls)} — "
                    f"Retrying (attempt {attempt}/{total_attempts}) …"
                )
            else:
                await Actor.set_status_message(
                    f"Place {current_seq}/{len(place_urls)} — Loading …"
                )

            # Create a fresh page from the shared context for this place.
            ...
            try:
                ...
                if attempt < total_attempts:
                    # Rotate browser + proxy immediately; _setup_browser closes old context
                    # (including this page), so we must not close it separately.
                    Actor.log.info("  Rotating proxy/browser for next attempt …")
        ...
        # Build request list and run crawler inside playwright lifecycle
        requests = [
